...
import { View, Animated, Dimensions, FlatList, PanResponder, StyleSheet } from "react-native";

import type { CarouselProps, GestureEvent, GestureState, ScrollEvent } from "../types";
...
export default class SideSwipe extends Component<CarouselProps, State> {
	...
	constructor(props: CarouselProps) {
		...
		const animatedValue: Animated.Value = Animated.divide(scrollPosAnim, itemWidthAnim);
	...
	render = () => {
		...
		return (
			<View style={[{ width: screenWidth }, style]} {...this.panResponder.panHandlers}>
				<AnimatedFlatList
					horizontal
					contentContainerStyle={[{ paddingHorizontal: contentOffset }, contentContainerStyle]}
					data={data}
					...
					onScroll={Animated.event([{ nativeEvent: { contentOffset: { x: scrollPosAnim } } }], {
	...
	handleGestureCapture = (e: GestureEvent, s: GestureState) => this.props.shouldCapture(s);

	handleGestureStart = (e: GestureEvent, s: GestureState) => this.props.onGestureStart(s);
	...
	handleGestureRelease = (e: GestureEvent, { dx, vx }: GestureState) => {
		...
		const resolvedIndex: number = Math.round(
			(resolvedOffset + (dx > 0 ? -this.props.threshold : this.props.threshold)) /
		...
		if (this.props.useVelocityForIndex) {
			...
			newIndex =
				dx > 0 ? Math.max(resolvedIndex, 0) : Math.min(resolvedIndex, this.props.data.length - 1);
		}
